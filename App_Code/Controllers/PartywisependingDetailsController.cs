...
public class PartywisependingDetailsController : ApiController
{
    [HttpPost]
    [ValidateModel]
    [Route("api/PartywisependingDetails")]
        ...
        try
        {
            ...
            if (mails.For == "Email")
            {
                if (dt.Rows.Count > 0)
                {
                    ...
                    da.AddEmailQueue(0, mails.To, mails.ToName, mails.CC, "", Subject, Body, dt.Rows[0]["EmailTime"].ToString(), dt.Rows[0]["EmailLastDate"].ToString(), mails.UserID, mails.LogNO, mails.ID, mails.Attachment, "Sales-Order-Pending", mails.SendNow);
                }
            }
            else
                if (mails.For == "SMS")
            {
                if (dt1.Rows.Count > 0)
                {
                    ...
                    da.AddMessageQueue(0, mails.To, mails.ToName, Body, dt1.Rows[0]["MessageTime"].ToString(), dt1.Rows[0]["LastMessageDate"].ToString(), mails.UserID, mails.LogNO, mails.ID, "Sales-Order-Pending", mails.SendNow);
                }
            }
