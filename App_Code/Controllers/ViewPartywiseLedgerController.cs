...
public class ViewPartywiseLedgerController : ApiController
{
    [HttpPost]
    [ValidateModel]
    [Route("api/ViewPartywiseLedger")]
    public HttpResponseMessage GetViewTemplate(PartywiseLedger mails)
    {
        ...
        try
        {
            ...
            List<DetailsTemplatePartywiseLedger> Template = new List<DetailsTemplatePartywiseLedger>();
            ...
            if (mails.For == "Email")
            {
                if (dt.Rows.Count > 0)
                {
                    ...
                    Template.Add(new DetailsTemplatePartywiseLedger
                    {
                        SubjectTEMP = Subject,
                        BodyTEMP = Body,
                    });
            ...
            else if (mails.For == "SMS")
            {
                if (dt1.Rows.Count > 0)
                {
                    Body = ms.ReplaceTemplatePartyWiseLedger(dt1.Rows[0]["Body"].ToString(), dt1.Rows[0]["Body"].ToString(), mails);
                }
                Template.Add(new DetailsTemplatePartywiseLedger
                {
                    SubjectTEMP = "",
