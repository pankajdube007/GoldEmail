...
public class TestPartyWiseLedgerController : ApiController
{
    [HttpPost]
    [ValidateModel]
    [Route("api/TestPartyWiseLedger")]
    public HttpResponseMessage GetTestMail(PartywiseLedger mails)
    {
        ...
        try
        {
            ...
            else if (mails.For == "SMS")
            {
                if (dt2.Rows.Count > 0)
                {
                    Body = ms.ReplaceTemplatePartyWiseLedger(dt2.Rows[0]["Body"].ToString(), dt2.Rows[0]["Body"].ToString(), mails).Replace("&", "and");
