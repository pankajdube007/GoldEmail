...
public class TestTemplatePartyWiseCDInvoiceController : ApiController
{
    [HttpPost]
    [ValidateModel]
    [Route("api/TestPartyWiseCDInvoice")]
    public HttpResponseMessage GetTestMail(PartyWiseCDInvoice mails)
    {
        ...
        try
        {
            ...
            else if (mails.For == "SMS")
            {
                if (dt2.Rows.Count > 0)
                {
                    Body = ms.ReplaceEmailTemplatePartyWiseCDInvoice(dt2.Rows[0]["Body"].ToString(), dt2.Rows[0]["Body"].ToString(), mails).Replace("&", "and");
