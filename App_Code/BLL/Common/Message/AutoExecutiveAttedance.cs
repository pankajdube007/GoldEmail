...
/// <summary>
/// Summary description for AutoExecutiveAttedance
/// </summary>
namespace Goldmedal.Emails.App_Code.BLL.Common.Message
{
    public class AutoExecutiveAttedance : ITask
    {
        ...
        public void Execute(XmlNode node)
        {
            ...
            if (DateTime.Now.Hour>=10 && DateTime.Now.Minute>=15)
            {
                DataTable dt = da.GetspfireDetails("aapexecsigninmissing");
                if (dt.Rows.Count == 0)
                {
                    try
                    {
                        ...
                        data.Add("Title", "Today attendance missing");
                        data.Add("Body", "Today Date: " + DateTime.Now.ToString("dd-MM-yyyy") + " punch in missing, Please Sign in...");
                        data.Add("Icon", "");
                        ...
                        DataTable dt1 = _messageService.return_dt("exec aapexecsigninmissing");
                        if (dt1.Rows.Count > 0)
                        {
                            ...
                            da.CreateSPFireDetails("aapexecsigninmissing");
                        }
                    }
                    catch (Exception exc)
                    {
