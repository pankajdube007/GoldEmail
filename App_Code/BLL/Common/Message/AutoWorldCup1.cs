...
/// <summary>
/// Summary description for AutoWorldCup1
/// </summary>
namespace Goldmedal.Emails.App_Code.BLL.Common.Message
{
    public class AutoWorldCup1 : ITask
    {
        ...
        public void Execute(XmlNode node)
        {
            ...
            {
                DataTable dt = da.GetspfireDetails("autosmswordcup1");
                if (dt.Rows.Count == 0)
                {

                    //  if (DateTime.Compare(DateTime.Now, time1) > 0)

                    DataTable dt1 = _messageService.return_dt("autosmswordcup1");
                    ...
                    if (dt1.Rows.Count > 0)
                    {
                        ...
                        for (int i = 0; i < dt1.Rows.Count; i++)
                        {
                            if (_messageService.ValidateMobileNo(dt1.Rows[i]["contactno"].ToString()))
                            {
                                ...
                                sms = "Dear "+ dt1.Rows[i]["name"].ToString() + ",Last few days to predict the winners of the matches to be played in the World Cup till the finals. We have checked from our records that you have not given any prediction for the league matches, Semi Final and Finals.So do hurry and get the maximum benefit by completing the prediction. There will be no selection allowed post 29th May 2019. Please ignore if you have filled the selection of the winners list.   Regards,  Team Goldmedal  *TC Apply";
                        ...
                        da.CreateSPFireDetails("autosmswordcup1");
                    }
